...
## Experiment-21: Architecture
     ...
     class Net(nn.Module):
        ...
        def forward(self, x):
            x_1 = self.pre_features(x)
            weights = self.weights(x_1) + x_1
            features = self.features(x_1) + x_1
            bias = self.bias(x_1) + x_1
            x_2 = weights * features + bias
            x_3 = self.post_features(x_2)
            x_4 = x_3.view(x_3.size(0), 512 * 8 * 8)
            x = self.classifier(x_4)
            return x

## Experiment-21: Hyperparamters
...
## Experiment-22: Architecture
     ...
     class Net(nn.Module):
        ...
        def forward(self, x):
            weights_32 = self.weights_32(x)
            bias_32 = self.bias_32(x)
            x = self.features_16(weights_32 * x + bias_32)
            weights_16 = self.weights_16(weights_32)
            bias_16 = self.bias_16(bias_32)
            x = self.features_8(weights_16 * x + bias_16)
            weights_8 = self.weights_8(weights_16)
            bias_8 = self.bias_8(bias_16)
            x = self.features_4(weights_8 * x + bias_8)
